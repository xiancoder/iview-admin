...
export default {
    ...
    props: {
        ...
        startVal: { type: Number, default: 0 }, // 起始值，即动画开始前显示的数值
        end: { type: Number, required: true }, // 结束值，即动画结束后显示的数值
        decimals: { type: Number, default: 0 }, // 保留几位小数
        decimal: { type: String, default: '.' }, // 分隔整数和小数的符号，默认是小数点
        duration: { type: Number, default: 2 }, // 动画持续的时间，单位是秒
        delay: { type: Number, default: 0 }, // 动画延迟开始的时间，单位是秒
        uneasing: { type: Boolean, default: false }, // 是否禁用easing动画效果
        usegroup: { type: Boolean, default: false }, // 是否使用分组，分组后每三位会用一个符号分隔
        separator: { type: String, default: ',' }, // 用于分组(usegroup)的符号
        simplify: { type: Boolean, default: false }, // 是否简化显示，设为true后会使用unit单位来做相关省略
        unit: { type: Array, default () { return [[3, 'K+'], [6, 'M+'], [9, 'B+']] } }, // 自定义单位，如[3, 'K+'], [6, 'M+']即大于3位数小于6位数的用k+来做省略 1000即显示为1K
    ...
    computed: {
        counterId () { return `count_to_${this._uid}` }
    },
    methods: {
