            ...
            <div>
                ...
                <FormItem label="用于职位" prop="post">
                    <Select class="wid250" v-model="formData.post" :multiple="true" filterable placeholder="请搜索 / 选择用于职位">
                        <Option v-for="row in usPosition" :value="row.id" :key="row.id" :label="row.name"></Option>
                    </Select>
                </FormItem>
...
export default {
    ...
    methods: {
        ...
        dataChange (e) {
            ...
            this.formData.post = []
            // 获取全部职位树
            this.$get('api/hr/getdeptpost', {
                deptId: e
            }).then(res => {
                this.usPosition = res.data.data.departments
            })
        },
