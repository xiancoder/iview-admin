        ...
        <div class="blog">
            ...
            <div class="blogContent" v-highlight>
                ...
                <div v-for="row in apis" class="mockapibox">
                    <p>h2. 1.1 用户管理-用户日志-用户日志-列表</p>
                    <p>&lt;pre&gt;
                        <Icon type="md-flower" size="20"/>
                    </p>
                    <p>&lt;code class="json"&gt;
                        <Icon type="md-flower" size="20"/>
                    </p>
                    ...
                    <p v-for="(row2,index) in row.params">________'<input v-show="row2.edit" type="text" v-model="row2.key"/><span v-show="!row2.edit">{{row2.key}}</span>':&nbsp;<input v-show="row2.edit" type="text" v-model="row2.value"/><span v-show="!row2.edit">{{row2.value}}</span>, //&nbsp;<input v-show="row2.edit" type="text" v-model="row2.dosc"/><span v-show="!row2.edit">{{row2.dosc}}</span>
                    ...
                    <p v-show="row.page">________'page_index': 1,
                        <Icon type="md-flower" size="20"/>
                    </p>
                    <p v-show="row.page">________'page_size': 30
                        <Icon type="md-flower" size="20"/>
                    </p>
                    <p>____},
                        <Icon type="md-flower" size="20"/>
                    ...
                    <p v-for="(row2,index) in row.list">________________'<input v-show="row2.edit" type="text" v-model="row2.key"/><span v-show="!row2.edit">{{row2.key}}</span>':&nbsp;<input v-show="row2.edit" type="text" v-model="row2.value"/><span v-show="!row2.edit">{{row2.value}}</span>, //&nbsp;<input v-show="row2.edit" type="text" v-model="row2.dosc"/><span v-show="!row2.edit">{{row2.dosc}}</span>
                        ...
                        <Icon type="md-ionic" size="20" @click="row2.type=2;row2.value='\'2020-4-27\'';row2.dosc='日期 yyyy-mm-dd'" title="日期 yyyy-mm-dd"/>
                        <Icon type="md-ionic" size="20" @click="row2.type=11;row2.value='\'2020-4-27 12:12\'';row2.dosc='日期 yyyy-mm-dd hh:mm'" title="日期 yyyy-mm-dd hh:mm"/>
                    ...
                    <p>____'params':{</p>
                    <p v-for="(row2) in row.list">________'<span>{{row2.key}}</span>':&nbsp;<span>{{row2.value}}</span>, //&nbsp;<span>{{row2.dosc}}</span>
                        <Icon type="md-flower" size="20"/>
                    </p>
                    <p>____},</p>
                    <p>____'response':{</p>
            ...
            apis: [
                {
                    ...
                    list: [
                        ...
                        {key: 'operate_at', value: '\'2020-4-27 12:12\'', dosc: '时间', edit: false, type: 11}
                    ],
                    page: true,
                    mock: null
                }
        ...
        makeMock (row) {
            ...
            for (var i = 0, l = row.list.length; i < l; i++) {
                ...
                switch (row.list[i].type) {
                    ...
                    case 11: x = '@date("yyyy-MM-dd hh:mm")'; break
                    default: x = '-'
                }
