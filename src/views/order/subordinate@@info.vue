...
<template>
    <div>
        ...
        <div style="margin-top: 10px">
            ...
            <Input style="width: 220px;margin-right: 10px" type="text" v-model="request.keyword" placeholder="标题、内容、姓名、组" @on-enter="handleSearch(1)"/>
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                { title: '发布时间', key: 'createdTimeStr', width: 50 },
                { title: '结单时间', key: 'updatedTimeStr', width: 150,
                    render: (h, params) => {
    ...
    methods: {
        ...
        handleSearch (v) {
            ...
            this.$router.replace({name: 'order-endList', query: obj})
            this.getList()
        },
        resetSearch () {
            this.$router.replace({name: 'order-endList'})
            this.request = {
                dates: this.dates,
                workType: '0',
                keyword: '',
        ...
        toInfo (row) {
            this.$router.push({name: 'order-info', query: {id: row.id}})
        }
    },
    mounted: function () {
        this.init()
