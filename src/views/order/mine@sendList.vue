...
export default {
    ...
    data () {
        return {
            dates: [],
            request: { dates: [], keyword: '', workType: '0', pageIndex: 1 },
            loading: false,
            rowcount: 0,
            columns: [
                ...
                { title: '最新回复人', width: 150, key: 'repleName',
                    render: (h, params) => {
    ...
    methods: {
        ...
        handleSearch (v) {
            ...
            this.$router.replace({name: 'order-sendList', query: obj})
            this.getList()
        },
        resetSearch () {
        ...
        toInfo (row) {
            this.$router.push({name: 'order-info', query: {id: row.id, handle: '1', end: '1'}})
        }
    },
    mounted: function () {
        this.init()
