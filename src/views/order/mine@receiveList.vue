...
<template>
    <div>
        ...
        <div style="margin-top: 10px">
            ...
            <Input style="width: 220px;margin-right: 10px" type="text" v-model="request.keyword" placeholder="标题、内容、发送人" @on-enter="handleSearch(1)"/>
...
export default {
    ...
    data () {
        return {
            dates: [],
            request: { dates: [], workType: '0', keyword: '', pageIndex: 1 },
            loading: false,
            rowcount: 0,
            columns: [
                { title: '发送人', key: 'sendName', width: 150 },
                ...
                { title: '发布时间', key: 'createdTimeStr', width: 150,
                    render: (h, params) => {
                        return h('span', params.row.createdTimeStr || '-')
                    }
                },
                { title: '更新时间', key: 'updatedTimeStr', width: 150,
                    render: (h, params) => {
                        return h('span', params.row.updatedTimeStr || '-')
                    }
                },
                { title: '最新回复人', key: 'repleName', width: 150,
                    render: (h, params) => {
    ...
    methods: {
        ...
        handleSearch (v) {
            ...
            this.$router.replace({name: 'order-receiveList', query: obj})
            this.getList()
        },
        resetSearch () {
            this.$router.replace({name: 'order-receiveList'})
            this.request = {
                dates: this.dates,
                workType: '0',
                keyword: '',
        ...
        toInfo (row) {
            this.$router.push({name: 'order-info', query: {id: row.id, handle: '1'}})
        }
    },
    mounted: function () {
        this.init()
